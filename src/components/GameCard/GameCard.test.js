...
describe('GameCard', () => {
    ...
    it('should display a line of lyrics with a missing word as an input box', () => {
        ...
        expect(getByText(`but I'll keep looking for Wonderland`, { exact: false })).toBeInTheDocument();
    })
    ...
    it('should increase score if input word is correct', () => {
        ...
        fireEvent.change(getByPlaceholderText('...'), {target: {value: 'Alice'}});
    ...
    it('should not increase score if input word is incorrect', () => {
        ...
        fireEvent.change(getByPlaceholderText('...'), {target: {value: 'Bobby'}});
        fireEvent.click(getByText('NEXT'));
        expect(mockUpdateCount).toHaveBeenCalledWith(false, splitLyric.missing.toUpperCase(), 'Bobby');
    })
